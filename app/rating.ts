...
export const LEVEL_RATINGS: Record<Level, number> = { 1: 1000, 2: 1400, 3: 1800 };
export const LEVEL_NAMES: Record<Level, string> = { 1: "Casual", 2: "Club", 3: "Expert" };
...
export const totalGames = (p: Profile) =>
  ([1, 2, 3] as Level[]).reduce((sum, l) => sum + p.records[l].w + p.records[l].l + p.records[l].d, 0);
...
export function makeDefaultProfile(): Profile {
  return {
    ...
    records: { 1: { w: 0, l: 0, d: 0 }, 2: { w: 0, l: 0, d: 0 }, 3: { w: 0, l: 0, d: 0 } },
...
export function loadProfile(): Profile {
  try {
    ...
    if (raw) return { ...makeDefaultProfile(), ...(JSON.parse(raw) as Profile) };
